  ...
  const customerRegistrySheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("顧客管理簿");
  const companyInformation = customerRegistrySheet.getDataRange().getValues();
...
// function KeywordSearch(){
  const keyword = quotationData[1][1]; //見積作成のB2を取得
  const suggestionRange = quotationData[0].slice(1, 11); // 見積作成B1からK1までのデータを取得
  const companyName = companyInformation.map(row => [row[0]]);  //顧客管理簿のA列のデータ取得
  const conpany = [];
  if(keyword === editedValue){
    for(i = 0; i <= companyName.length; i++){
      if (companyName[i].includes(keyword)) {
        const matchedCompanyName = companyName[i];
        const matchedSuggestionRange = suggestionRange.map(value => `${matchedCompanyName}: ${value}`);
        Logger.log(matchedSuggestionRange);
